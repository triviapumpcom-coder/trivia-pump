...
import type { ChoiceLetter } from "../store/game";
...
export function QuestionCard({ question, options, media, category, difficulty }: QuestionCardProps): JSX.Element {
  ...
  return (
    <div className="rounded-3xl bg-black/40 p-3 sm:p-4 lg:p-6 shadow-2xl border border-white/20">
      {/* Category Badge */}
      {category && (
        <CategoryBadge category={category} difficulty={difficulty} />
          ...
          {hasMedia ? (
            <div className="w-full mb-3 lg:mb-4 flex justify-center">
              <img 
                src={media?.[0]?.url} 
                alt="Question image" 
                className="rounded-2xl max-h-48 sm:max-h-56 lg:max-h-64 max-w-full object-contain shadow-lg" 
              />
            </div>
          ) : null}
      
      <div className="text-center mb-4 lg:mb-6">
        <h2 className={`font-black text-white leading-tight mb-2 lg:mb-3 ${
          hasMedia 
        ...
        <p className={`text-white/80 font-semibold ${
          ...
          Answer in live chat: <span className="text-green-400">/a /b /c /d</span>
        </p>
      </div>
      
      <div className={`grid grid-cols-1 lg:grid-cols-2 ${hasMedia ? 'gap-2 sm:gap-3 lg:gap-4' : 'gap-3 sm:gap-4 lg:gap-6'}`}>
        {options.map((opt, idx) => {
          ...
          const getOptionFontSize = (text: string, hasMedia: boolean | undefined) => {
          ...
          return (
            ...
            >
              ...
              <div
                className="absolute inset-0 bg-gradient-to-r from-green-500/30 to-blue-500/30 transition-all duration-300"
                style={{ width: `${Math.max(pct, 2)}%` }}
              />
              <div className={`relative ${hasMedia ? 'p-2 sm:p-3 lg:p-4' : 'p-3 sm:p-4 lg:p-6'}`}>
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-start gap-2 lg:gap-3 flex-1 min-w-0">
                    <span className={`text-white bg-white/20 rounded-full flex items-center justify-center font-black flex-shrink-0 mt-1 ${
                      hasMedia 
                        ? 'w-8 h-8 text-sm sm:w-9 sm:h-9 sm:text-base lg:w-10 lg:h-10 lg:text-lg' 
                        : 'w-9 h-9 text-base sm:w-10 sm:h-10 sm:text-lg lg:w-12 lg:h-12 lg:text-2xl'
                    }`}>
                      {letter}
                    </span>
                    <span className={`text-white font-bold leading-tight ${getOptionFontSize(opt, hasMedia)}`}>
                      {opt}
                    </span>
                  </div>
                  <div className="text-right flex-shrink-0 ml-2">
                    ...
                    <div className={`text-white/60 ${hasMedia ? 'text-xs' : 'text-sm'}`}>{count} votes</div>
                  </div>
                </div>
              </div>
            </div>
